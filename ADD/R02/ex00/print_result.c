...
/*   Updated: 2024/02/13 17:37:57 by svan-de-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	hundreds(t_data *data, int index)
{
	...
	write(1, data->dict[data->number[index] - '0'],
		ft_strlen(data->dict[data->number[index] - '0']));
	write(1, " ", 1);
...
void	tens(t_data *data, int index)
{
	...
	if (data->number[index] == '1')
		write(1, data->dict[10 + data->number[index + 1] - '0'],
			ft_strlen(data->dict[10 + data->number[index + 1] - '0']));
	else
	{
		write(1, data->dict[20 + (data->number[index] - '0' - 2)],
			ft_strlen(data->dict[20 + (data->number[index] - '0' - 2)]));
	}
	data->writted = 1;
	data->no_print = 0;
}
...
void	greatness(t_data *data, int len)
{
	if (data->writted == 1)
		(write(1, " ", 1), data->writted = 0);
	write(1, data->dict[29 + ((len - 1) / 3)],
		ft_strlen(data->dict[29 + ((len - 1) / 3)]));
	data->writted = 1;
	data->no_print = 1;
}
